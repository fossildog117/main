...
abstract class Assistant {
    ...
    public static int retentionStrengthBias;
    ...
    static boolean CompareSubject(String line) {
        ...
        for (String word : Assistant.GetWords(line)) {
            ...
            if (index >= 0) {
                ...
                double recalProbability = Math.pow(2.7183, -(subject.getTimeSinceLastRecall() / (retentionStrengthBias + subject.getRetentionStrength())));
    ...
    static void PrintList() {
        ...
        try {
            ...
            for (Subject s : SubjectList) {
                if (s.getRetentionStrength() >= 1000) {
    ...
    static void PrintList(int n, SearchEngine searchEngine) {
        ...
        try {
            ...
            PrintWriter writer = new PrintWriter(System.getProperty("user.dir") + "/furnitureOutput" + n + "_" + searchEngine.getStopper() +".txt", "UTF-8");

            for (Subject s : SubjectList) {
                if (s.getRetentionStrength() >= 10) {
                    writer.println(s.getSubjectName().toUpperCase() + "," + s.getTimeSinceLastRecall() + "," + s.getRetentionStrength());
                }
            }
